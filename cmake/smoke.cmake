...
#
# Drives the linux-sanitize / linux-ubsan CI jobs' smoke steps AND the
# `smoke_*` ctest entries registered in CMakeLists.txt (when
# UMBRA_BUILD_EXAMPLES=ON and either UMBRA_SANITIZE=ON or MSVC), so CI and
# developers share one implementation of the acceptance rule instead of a
# copy-pasted shell loop:
#
...
#   - the binary is killed after 5 seconds by CMake's own execute_process
#     TIMEOUT (no external `timeout` binary needed - the Windows CI job runs
#     this same script, and Windows has no GNU timeout in its default PATH:
#     C:\Windows\System32\timeout.exe is a different, incompatible command);
#   - "Process terminated due to timeout" (killed after the full 5s window)
#     = the app looped through its update/render paths without crashing - the
#     expected pass for these infinite-loop apps. An immediate exit 0 is
#     treated as failure too: it only happens when start() failed (e.g.
#     renderer/audio init), i.e. zero code paths covered, so exit 0 is
#     accepted only when the process demonstrably ran for at least 2 seconds.
#     Any other non-zero exit is a crash or sanitizer finding (ASan aborts
...
# Same env as the CI smoke steps: SDL dummy drivers, autoplay for games, and
# the sanitizer options exactly as CI sets them. Linux enables LSan
# (detect_leaks=1) just like the CI job; macOS clang ships no LeakSanitizer,
# where detect_leaks=1 makes ASan abort every binary with "detect_leaks is
# not supported on this platform" - so it is omitted there (same platform
# note as cmake/sanitize.cmake). Windows is not a sanitizer build, so the
# options are inert. Memory errors and UB are still caught on Linux/macOS.
...
# CMake's native TIMEOUT replaces GNU `timeout`: it kills the child after 5s
# cross-platform (CreateProcess/TerminateProcess on Windows, signal on POSIX)
# and reports "Process terminated due to timeout" in RESULT_VARIABLE.
...
if(run_result STREQUAL "Process terminated due to timeout")
...
elseif(run_result EQUAL 0 AND _elapsed GREATER_EQUAL 2)
    ...
    message(FATAL_ERROR
        "smoke FAILED on ${SMOKE_TARGET} (result '${run_result}' after ${_elapsed}s) - "
